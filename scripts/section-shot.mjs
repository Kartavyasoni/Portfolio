...
await p.evaluate(() => { document.documentElement.style.scrollBehavior='auto'; document.querySelectorAll('[data-reveal]').forEach(e=>e.style.opacity='1'); const el=document.getElementById('projects'); el?.scrollIntoView(); });
await new Promise(r => setTimeout(r, 2600));
await p.screenshot({ path: '/tmp/projects.png' });
console.log('projects → /tmp/projects.png');
await b.close();
